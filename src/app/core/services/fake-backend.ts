...
export class FakeBackendInterceptor implements HttpInterceptor {
  intercept(
    ...
    function handleRoute() {
      switch (true) {
        ...
        case url.match(/\/accounts\/.+$/) && method === 'GET':
          return getUserById();
        case url.match(/\/accountById\/.+$/) && method === 'GET':
    ...
    function getAccounts() {
      ...
      if (paramSearch.age !== 0) {
        rs = rs.filter((x: Account) => x.age == paramSearch.age);
      }
      if (paramSearch.balance !== 0) {
        rs = rs.filter((x: Account) => x.balance == paramSearch.balance);
      }
