...
export class AccountService {
  ...
  getAccounts(param: ParamSearch): Observable<any> {
    ...
    params = params.append('age', param.age ? param.age.toString() : '');
    params = params.append('balance', param.balance ? param.balance.toString() : '');
    return this.http.get<Account[]>('/accounts', {
      params
    });
  }

  addAccount(acc: AccountData): Observable<any> {
    return this.http.post('/accounts', acc);
  }

  getAllAccount(): Observable<any> {
    return this.http.get('/accounts/all');
  }

  editAccount(acc: AccountData): Observable<any> {
  ...
  getAccountById(id: string) {
    return this.http.get('/accountById/' + id);
  }
}
